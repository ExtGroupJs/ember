...
$("#modal-eliminar-elemento").on("show.bs.modal", function (event) {
  ...
  modal
    .find(".modal-body")
    .text("¿Desea eliminar la responsabilidad: " + dataName + "?");
});
...
function function_delete(selected_id) {
  ...
  axios
    ...
    .then((response) => {
      Toast.fire({
        icon: "success",
        title: "La responsabilidad se eliminó correctamente",
      });
    ...
    .catch((error) => {
      Toast.fire({
        icon: "error",
        title: "La responsabilidad no se eliminó",
      });
    });
}
...
$(function () {
  ...
  $("#form-create-user").validate({
    ...
    messages: {
      name: {
        required: "Por favor debe ingresar un nombre",
      },
    },
    errorElement: "span",
