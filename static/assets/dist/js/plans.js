    ...
    if (button.data('type') == "edit") {
        ...
        axios.get(`${url}${selected_id}/`)
            .then(function (response) {
                ...
                $('#classification').val(element.product_kind.id).trigger('change.select2');          
                form.elements.quantity.value = element.quantity;
                $('#measurement_unit').val(element.measurement_unit.id).trigger('change.select2');
...
form.addEventListener('submit', function (event) {
    ...
    data.append("product_kind", document.getElementById("classification").value);
    data.append("ueb", document.getElementById("entity").value);
    data.append("quantity", document.getElementById("quantity").value);
    data.append("measurement_unit", document.getElementById("measurement_unit").value);
...
function poblarListas() {
    ...
    axios.get("/product-gestion/measurement-unit/?used_for_planning=true").then(function (response) {
        response.data.results.forEach(function (element) {
            var option = new Option(element.name, element.id);
