    ...
    if (button.data('type') == "edit") {
        ...
        axios.get(`${url}${selected_id}/`)
            .then(function (response) {
                ...
                $('#distribution_format').val(elemento.distribution_format.id).trigger('change.select2');
                ...
                $('#plans').select2({ dropdownParent: $('#modal-crear-elemento'), theme: 'bootstrap4' });
...
function loadProduct(id) {
    ...
    $('#product').empty();
    $('#product').trigger('change');
    axios.get("/product-gestion/plan/" + id + "/allowed-products/").then(function (response) {
