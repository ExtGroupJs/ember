...
from apps.products_app.models import (Classification, GroupingPackaging,
                                      IndividualPackaging)
...
def test_creating_individual_packaging():
    ...
    individual_packages_created = baker.make("products_app.IndividualPackaging", _quantity=quantity)
...
def test_deleting_individual_packaging():
    quantity = random.randint(1, 100)
    baker.make("products_app.IndividualPackaging", is_grouping_packaging=True, _quantity=quantity)
    assert IndividualPackaging.objects.count() == quantity
    assert GroupingPackaging.objects.count() == quantity
    assert IndividualPackaging.objects.filter(historical_vault__isnull=True).count() == quantity
    assert GroupingPackaging.objects.filter(historical_vault__isnull=True).count() == quantity
...
def test_deleting_individual_packaging_not_allowed_if_is_used():
    ...
    baker.make("products_app.Production", distribution_format__individual_packaging=individual_package, _quantity=quantity)
    assert GroupingPackaging.objects.count() == quantity
    assert IndividualPackaging.objects.count() == 1
    assert IndividualPackaging.objects.filter(historical_vault__isnull=True).count() == 1
    assert GroupingPackaging.objects.filter(historical_vault__isnull=True).count() == quantity
...
def test_creating_clasification(client):
    ...
    clasifications_with_parents = Classification.objects.filter(parent__isnull=False).count()
    url_with_filter = base_url + f"?parent__isnull=False"
